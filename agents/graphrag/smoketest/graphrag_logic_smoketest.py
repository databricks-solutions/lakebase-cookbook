...
def main() -> int:
    ...
    check(
        "substitute edge stored once (sorted)",
        ("product:122", "product:130", "SUBSTITUTE_FOR") in bedges
        and ("product:130", "product:122", "SUBSTITUTE_FOR") not in bedges,
        True,
    )
    ...
    check(
        "valid substitute still present",
        ("product:122", "product:130", "SUBSTITUTE_FOR") in gedges,
        True,
    )
    check(
        "hallucinated substitute 999 dropped",
        any("999" in s or "999" in d for (s, d, _r) in gedges),
        False,
    )
    ...
    check(
        "floor 0.55: on-topic seed still ranks first",
        floored[0]["node_id"] if floored else None,
        "product:122",
    )
    check("floor 0.55: narrows the result set", len(floored) < len(wide), True)
    check(
        "floor 0.55: keeps the on-topic supplier via graph expansion",
        "supplier:S2" in floored_ids,
        True,
    )
    ...
    check("default floor reproduces the pre-floor result set", sorted(wide_ids), expected_unfloored)
    ...
    check(
        "floor above max similarity returns nothing",
    ...
    check(
        "all-zero embedding never becomes a seed",
        "product:261" not in [r["node_id"] for r in zeroed[:1]],
        True,
    )
    ...
    doc = (
        "Acme Foods supplies skim milk. ACME Foods Inc. is based in Boston. "
    ...
    check(
        ...
        [p.chunk_id for p in passages][:2],
        ["doc:1:c0", "doc:1:c1"],
    )
    check("every passage keeps its doc pointer", all(p.doc_id == "doc:1" for p in passages), True)
    ...
    check("text without sentence terminators is still split", len(unpunctuated) > 1, True)
    ...
    check(
        "no passage approaches the whole document",
        max(len(p.text) for p in unpunctuated) <= 2 * 60,
        True,
    )
    check(
        "a 1000-char unpunctuated run yields many passages, not one", len(unpunctuated) >= 10, True
    )
    check(
        "overlap does not start a passage mid-word",
        all(not p.text.startswith(" ") for p in passages),
        True,
    )

    # -- trigram measures
    check(
        "alias pair scores above the merge threshold",
        trigram_sim("Acme Foods", "ACME Foods Inc.") >= 0.55,
        True,
    )
    check(
        "unrelated pair scores below the merge threshold",
        trigram_sim("Acme Foods", "Diesel Generator") < 0.55,
        True,
    )
    check(
        "trigram similarity is symmetric",
        round(trigram_sim("Acme", "ACME Foods"), 6) == round(trigram_sim("ACME Foods", "Acme"), 6),
        True,
    )
    check(
        "Jaccard MISSES the legal-suffix alias (why containment exists)",
        trigram_sim("Acme Foods", "Acme Foods Incorporated") < 0.55,
        True,
    )
    check(
        "containment catches the legal-suffix alias",
        trigram_containment("Acme Foods", "Acme Foods Incorporated") >= 0.8,
        True,
    )
    check(
        "containment still rejects an unrelated pair",
        trigram_containment("Acme Foods", "Diesel Generator") < 0.8,
        True,
    )
    ...
    company_ids = {
        res.canonical_id[(n, "Company")] for n in ("Acme Foods", "ACME Foods Inc.", "Acme")
    }
    check("three alias spellings collapse to ONE canonical id", len(company_ids), 1)
    check(
        "canonical display name is the fullest spelling",
        res.canonical_name[company_ids.pop()],
        "ACME Foods Inc.",
    )
    check("each merged alias is recorded for audit", len(res.same_as), 2)
    check(
        "canonical ids carry a type prefix (graph_build convention)",
        res.canonical_id[("Acme Foods", "Company")].startswith("company:"),
        True,
    )
    ...
    check(
        "same name + different type get DIFFERENT canonical ids",
        dres.canonical_id[("Acme Foods", "Company")]
        != dres.canonical_id[("Acme Foods", "Product")],
        True,
    )
    ...
    check(
        "no node id carries two conflicting types", all(len(v) == 1 for v in by_id.values()), True
    )
    ...
    check(
        "every row carries exactly the 8 contract columns",
        all(set(r) == CONTRACT for r in rows),
        True,
    )
    check(
        "relations are attributed to llm_extract",
        {r["source_method"] for r in rows if r["predicate"] != "SAME_AS"},
        {"llm_extract"},
    )
    check(
        "alias edges are attributed to entity_resolution",
        {r["source_method"] for r in rows if r["predicate"] == "SAME_AS"},
        {"entity_resolution"},
    )
    check(
        "subjects/objects were rewritten to canonical ids",
        all(":" in r["subject_id"] and ":" in r["object_id"] for r in rows),
        True,
    )
    ...
    check("SAME_AS rows carry the resolved type, not 'Entity'", "Entity" not in same_as_types, True)
    ...
    check(
        "no self-referential rows (would break the edges primary key)",
        [r for r in vrows if r["subject_id"] == r["object_id"]],
        [],
    )
    check(
        "no duplicate (subject, predicate, object) rows",
        len({(r["subject_id"], r["predicate"], r["object_id"]) for r in vrows}),
        len(vrows),
    )
    ...
    check(
        "malformed mentions are skipped, valid one survives",
        [r["predicate"] for r in jrows],
        ["LOCATED_IN"],
    )
    ...
    check("SUBSTITUTE_FOR is declared symmetric", "SUBSTITUTE_FOR" in SYMMETRIC_PREDICATES, True)
    # SAME_AS is directional by design (alias -> canonical) and must NOT be symmetric
    check(
        "SAME_AS is NOT symmetric (stays directional)", "SAME_AS" not in SYMMETRIC_PREDICATES, True
    )

    # -- relation vocabulary, normalized before the membership test
    check(
        "normalize_predicate uppercase-snakes a raw phrase",
        normalize_predicate("is based in"),
        "IS_BASED_IN",
    )
    formatted = [
    ...
    dropped = build_gold_triplets(
        formatted, fres, allowed_predicates=DEFAULT_PREDICATES, on_unknown="drop"
    )
    ...
    check(
        "lowercase/spaced in-vocabulary predicates survive the allowlist",
        preds_kept,
        ["LOCATED_IN", "SUPPLIED_BY"],
    )
    kept_all = build_gold_triplets(formatted, fres)
    check(
        "default keeps out-of-vocabulary predicates",
        "IS_PRIMARY_DAIRY_SUPPLIER_IN" in {r["predicate"] for r in kept_all},
        True,
    )
    check(
        "the example's relation vocabulary matches sql/schema.sql",
        DEFAULT_PREDICATES,
    ...
    check("end-to-end rows conform to the contract", all(set(r) == CONTRACT for r in e2e), True)
    ...
    check(
        "duplicate relations across chunks are de-duplicated",
        len([r for r in e2e if r["predicate"] == "LOCATED_IN"]),
        1,
    )
    check(
        "canonical_id is a stable, type-prefixed slug",
        canonical_id("ACME Foods Inc.", "Company"),
        "company:acme_foods_inc",
    )
    ...
    # further down.
    check(
        "unmarked nodes report source_class=inferred",
        {r["source_class"] for r in baseline},
        {"inferred"},
    )
    check(
        "unmarked nodes: authority_score == score (shape, not a guard)",
        all(r["authority_score"] == r["score"] for r in baseline),
        True,
    )
    ...
    certified = {
        nid: (dict(n, props={"source_method": "uc_certified"}) if nid == runner_up else n)
        for nid, n in bnodes.items()
    }
    default_boost = agent_retrieve(q, certified, bedges, a_emb, seed_k=6, max_hops=2)
    check(
        "authority_boost=1.0 is an exact no-op with certified rows present",
        [r["node_id"] for r in default_boost],
        baseline_ids,
    )
    check(
        "certified row is labelled",
    ...
    check(
        "authority gap fixture is positive (guards the ratio below)",
        baseline[0]["score"] > 0 and baseline[1]["score"] > 0,
        True,
    )
    gap = baseline[0]["score"] / baseline[1]["score"]
    strong = agent_retrieve(
        q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=gap * 1.10
    )
    check(
        "a sufficient boost promotes the certified node to rank 1", strong[0]["node_id"], runner_up
    )
    ...
    check(
        "weakest-node fixture is positive (guards the ratio below)", baseline[-1]["score"] > 0, True
    )
    weak_gap = baseline[0]["score"] / baseline[-1]["score"]
    certified_weak = {
        nid: (dict(n, props={"source_method": "uc_certified"}) if nid == weakest else n)
        for nid, n in bnodes.items()
    }
    modest = agent_retrieve(
        q, certified_weak, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=1.2
    )
    check(
        "a modest boost does NOT let the least relevant certified node reach rank 1",
        modest[0]["node_id"] == weakest,
        False,
    )
    ...
    check(
        "SQL: boost=1.0 orders by raw score (no-op)",
        [r[0] for r in base_sql],
        ["inf:hi", "cert:hi", "bare:mid", "other:mid", "empty:mid", "cert:neg", "inf:neg"],
    )
    check(
        "SQL: a node with no source_method reads as inferred, weight 1.0",
        next((r[1], r[2]) for r in base_sql if r[0] == "bare:mid"),
        ("inferred", 0.3),
    )
    check(
        "SQL: a foreign source_method passes through verbatim, not normalised",
        next(r[1] for r in base_sql if r[0] == "other:mid"),
        "structured",
    )
    ...
    check(
        "SQL: a present-but-empty source_method stays empty, not 'inferred'",
    ...
    check(
        "SQL: boost promotes the certified node past the stronger inferred one",
        [r[0] for r in boosted][:2],
        ["cert:hi", "inf:hi"],
    )
    ...
    check(
        "SQL: boosting never demotes a negative-score certified node", neg_after <= neg_before, True
    )
    check(
        "SQL: the negative certified node keeps its unboosted score",
        next(r[2] for r in boosted if r[0] == "cert:neg"),
        -0.1,
    )
    ...
    neg_nodes = {
        "cert:neg": {
            "node_type": "N",
            "name": "certified, anti-correlated",
    ...
    nq = [1.0, 0.0]
    neg_base = agent_retrieve(nq, neg_nodes, [], neg_emb, seed_k=2, max_hops=0, seed_floor=-1.0)
    neg_boost = agent_retrieve(
        nq, neg_nodes, [], neg_emb, seed_k=2, max_hops=0, seed_floor=-1.0, authority_boost=2.0
    )
    check(
        "py: negative-score fixture really is negative", all(r["score"] < 0 for r in neg_base), True
    )
    check(
        "py: the certified node starts ABOVE the inferred one (else nothing can demote it)",
        [r["node_id"] for r in neg_base],
        ["cert:neg", "inf:neg"],
    )
    check(
        "py: boosting never demotes a negative-score certified node",
        [r["node_id"] for r in neg_boost],
        ["cert:neg", "inf:neg"],
    )
    ...
    check(
        "py: the negative certified node's AUTHORITY score is left unboosted",
    ...
    tie_nodes = {
        "z:hi": {"node_type": "N", "name": "z", "props": {}},
        "a:lo": {"node_type": "N", "name": "a", "props": {}},
    }
    tie_emb = {"z:hi": [1.0, 0.0], "a:lo": [0.99999, 0.00447]}
    tied = agent_retrieve([1.0, 0.0], tie_nodes, [], tie_emb, seed_k=2, max_hops=0, seed_floor=-1.0)
    check("py: the tie fixture rounds to one value at 4 dp", len({r["score"] for r in tied}), 1)
    check(
        "py: but the raw scores differ, so ordering is by the unrounded key",
        [r["node_id"] for r in tied],
        ["z:hi", "a:lo"],
    )

    # (iii) a None boost must coalesce to 1.0 like the SQL, not raise.
    none_boost = agent_retrieve(
        q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=None
    )
    check(
        "py: authority_boost=None behaves as 1.0 (matches SQL COALESCE)",
        [r["node_id"] for r in none_boost],
        baseline_ids,
    )
    ...
    empty_out = agent_retrieve(
        [1.0, 0.0], empty_nodes, [], {"e:1": [1.0, 0.0]}, seed_k=1, max_hops=0, seed_floor=-1.0
    )
    check(
        "py: a present-but-empty source_method stays empty, not 'inferred'",
        empty_out[0]["source_class"],
        "",
    )

    # (iii-b) NaN must be treated as absent, not slipped past the clamp.
    nan_boost = agent_retrieve(
        q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=float("nan")
    )
    check(
        "py: authority_boost=NaN behaves as 1.0, not as a demotion",
        [r["node_id"] for r in nan_boost],
        baseline_ids,
    )
    check(
        "py: NaN never reaches authority_score (would not be valid JSON)",
        all(r["authority_score"] == r["authority_score"] for r in nan_boost),
        True,
    )

    # (iv) a boost BELOW 1.0 must not demote either — the clamp, not just the sign guard.
    for bad in (0.5, 0.0, -2.0):
        clamped = agent_retrieve(
            q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=bad
        )
        check(
            f"py: boost={bad} is clamped, certified node not demoted",
            [r["node_id"] for r in clamped],
            baseline_ids,
        )
    lo = authority_sql(0.5)
    check(
        "SQL: boost=0.5 is clamped, ordering unchanged",
        [r[0] for r in lo],
        [r[0] for r in base_sql],
    )
    neg_sql = authority_sql(-2.0)
    check(
        "SQL: boost=-2.0 is clamped, ordering unchanged",
        [r[0] for r in neg_sql],
        [r[0] for r in base_sql],
    )
    ...
    for nf, label in [
        ...
        check(
            f"SQL: boost={label} is treated as absent, ordering unchanged",
            [r[0] for r in nf_sql],
            [r[0] for r in base_sql],
        )
        check(
            f"SQL: no non-finite authority_score escapes at boost={label}",
            all(r[2] is not None and math.isfinite(float(r[2])) for r in nf_sql),
            True,
        )

        py_nf = agent_retrieve(
            q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=nf
        )
        check(
            f"py: boost={label} is treated as absent, ordering unchanged",
            [r["node_id"] for r in py_nf],
            baseline_ids,
        )
        check(
            f"py: authority_score stays JSON-serializable at boost={label}",
            "Infinity" not in json.dumps([r["authority_score"] for r in py_nf])
            and all(math.isfinite(r["authority_score"]) for r in py_nf),
            True,
        )
    ...
    try:
        ...
        dec = agent_retrieve(
            q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=Decimal("2.0")
        )
        dec_ids = [r["node_id"] for r in dec]
    except Exception as err:
        dec_ids = f"{type(err).__name__}: {err}"
    check(
        "py: a Decimal boost ranks identically to the same float",
        dec_ids,
        [r["node_id"] for r in flt],
    )
    ...
    for odd, label in [
        ...
        try:
            got = agent_retrieve(
                q, certified, bedges, a_emb, seed_k=6, max_hops=2, authority_boost=odd
            )
            ok = all(
                isinstance(r["authority_score"], float) and math.isfinite(r["authority_score"])
                for r in got
            )
