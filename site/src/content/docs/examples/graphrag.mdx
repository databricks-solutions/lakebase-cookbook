...
4. **Authority** — a node marked as curated in `props` (`{"source_method": "uc_certified"}`)
   ...
   - **`source_class` is passed through verbatim**, not normalised to two values, so any
     producer's label survives. In practice today it is `uc_certified` or `inferred`, since
     nothing writes `source_method` into node props yet. Only `uc_certified` is boosted;
