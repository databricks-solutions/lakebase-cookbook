...
4. **Authority** — a node marked as curated in `props` (`{"source_method": "uc_certified"}`)
   ...
   - **Only positive scores are boosted.** `seed_similarity` spans `[-1, 1]` and the
     ...
     promote.
   - **`source_class` is passed through verbatim**, not normalised to two values — the
     `gold_triplets` convention already emits `structured` and `llm_enrichment`. Only
     `uc_certified` is boosted; everything else carries weight `1.0`. Branch on equality
     with `uc_certified`, never on inequality with `inferred`.
