...
- **`information_schema.semantic_*` is not populated for native metric views.** `semantic_views`,
  ...
  holding a real Databricks metric view exposed none — so they appear to describe federated
  Snowflake semantic views. Reading them finds nothing for a Databricks metric view today.
