...
4. **Authority** — a node marked as curated in `props` (`{"source_method": "uc_certified"}`)
   ...
   Populating those certified nodes — from Unity Catalog semantics or any other curated
   source — is deliberately outside this example: anything that can write the `props` key
   participates, which keeps the ranking independent of any one producer.
