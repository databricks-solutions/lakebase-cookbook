...
| `seed_floor` | Minimum cosine similarity a semantic seed must clear before it enters the graph walk. Cosine ranges `[-1, 1]`, so the default `-1.0` keeps every seed (identical to no floor). Raise it toward `0.3` on distractor-heavy corpora, where weak seeds bridge into unrelated subgraphs and dilute precision. |
